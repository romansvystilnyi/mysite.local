...
namespace RedBeanPHP\Driver {
    ...
    class RPDO implements Driver
    {
        ...
        public function __construct($dsn, $user = null, $pass = null)
        {
            ...
            if ((strpos($this->dsn,
                        'sqlite') === 0 && PHP_MAJOR_VERSION === 5 && PHP_MINOR_VERSION === 3) || defined('HHVM_VERSION') || $this->dsn === 'test-sqlite-53') {
...
namespace RedBeanPHP {
    ...
    class OODBBean implements \IteratorAggregate, \ArrayAccess, \Countable, Jsonable
    {
        ...
        public function __set($property, $value)
        {
            ...
            $exists = isset($this->properties[$property]);
            $fieldLink = $property . '_id';
            $isFieldLink = (($pos = strrpos($property,
                        '_id')) !== false) && array_key_exists(($fieldName = substr($property, 0, $pos)),
                    $this->properties);
        ...
        public function __call($method, $args)
        {
            ...
            if (!method_exists($this->__info['model'], $method)) {
                ...
                if (in_array($method, array('update', 'open', 'delete', 'after_delete', 'after_update', 'dispense'),
                    true)) {
                    return null;
                }
        ...
        public function countOwn($type)
        {
            ...
            if ($this->getID()) {
                ...
                if (!self::$useFluidCount) {
                    $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount($type, array(), $sql,
                        $bindings);
                } else {
        ...
        public function countShared($type)
        {
            ...
            $count = 0;
            if ($this->getID()) {
                $count = $redbean->getAssociationManager()->relatedCount($this, $type, $this->withSql,
                    $this->withParams);
            }
            $this->clearModifiers();
    ...
    abstract class AQueryWriter
    {
        ...
        private function makeSQLFromConditions($conditions, &$bindings, $addSql = '')
        {
            ...
            foreach ($conditions as $column => $values) {
                ...
                if ($checkOODB instanceof OODBBean && $checkOODB->getMeta('type') === $column && substr($column,
                        -3) != '_id') {
                    $column = $column . '_id';
                }
        ...
        public function parseJoin($type, $sql, $cteType = null)
        {
            ...
            foreach ($expressions as $exp) {
                ...
                while (true) {
                    ...
                    if ($i) {
                        $joinSql .= $this->writeJoin($joinTable, $joinInfo, 'INNER', $joinType, false, "__rb{$nsuffix}",
                            $aliases, null);
                    } else {
                        $joinSql .= $this->writeJoin($joinTable, $joinInfo, 'LEFT', $joinType, true, "__rb{$nsuffix}",
                            $aliases, $cteType);
                    }
        ...
        {
            list($sourceTable, $destTable, $linkTable, $sourceCol, $destCol) = $this->getRelationalTablesAndColumns($sourceType,
                $destType);
            ...
            if ($sourceType === $destType) {
                $inClause2 = $this->getParametersForInClause($linkIDs, $bindings,
                    count($bindings)); //for some databases
                $sql = "
			SELECT
        ...
        public function queryCountTagged($type, $tagList, $all = false, $addSql = '', $bindings = array())
        {
            $rows = $this->queryTaggedGeneric($type, $tagList, $all, $addSql, $bindings,
...
namespace RedBeanPHP\QueryWriter {
    ...
    class CUBRID extends AQueryWriter implements QueryWriter
    {
        ...
        protected function getKeyMapForType($type)
        {
            ...
            $matches = array();
            preg_match_all('/CONSTRAINT\s+\[([\w_]+)\]\s+FOREIGN\s+KEY\s+\(\[([\w_]+)\]\)\s+REFERENCES\s+\[([\w_]+)\](\s+ON\s+DELETE\s+(CASCADE|SET\sNULL|RESTRICT|NO\sACTION)\s+ON\s+UPDATE\s+(SET\sNULL|RESTRICT|NO\sACTION))?/',
                $sqlCode[0]['CREATE TABLE'], $matches);
            $list = array();
            if (!isset($matches[0])) {
                return $list;
            }
            ...
            foreach ($bean as $property => $value) {
                ...
                if ($value instanceof OODBBean) {
                    ...
                    if (strpos($property, 'own') === 0) {
                        list($ownAdditions, $ownTrashcan, $ownresidue) = $this->processGroups($originals, $value,
                            $ownAdditions, $ownTrashcan, $ownresidue);
                        $listName = lcfirst(substr($property, 3));
                        ...
                        list($sharedAdditions, $sharedTrashcan, $sharedresidue) = $this->processGroups($originals,
                            $value, $sharedAdditions, $sharedTrashcan, $sharedresidue);
                        unset($bean->$property);
                    }
                }
            }
    ...
    class Fluid extends Repository
    {
        ...
        private function modifySchema(OODBBean $bean, $property, $value, &$columns = null)
        {
            ...
            if (!$this->oodb->isChilled($bean->getMeta('type'))) {
                ...
                if (isset($columns[$this->writer->esc($property,
                        true)])) { //Is this property represented in the table ?
                ...
                if ($doFKStuff) {
                    if (strrpos($columnNoQ, '_id') === (strlen($columnNoQ) - 3)) {
                        ...
                        if (is_array($isLink) && !count(array_diff($isLink,
                                array_keys($this->writer->getColumns($table))))) {
        ...
        protected function storeBean(OODBBean $bean)
        {
            if ($bean->getMeta('changed')) {
                ...
                $partial = ($this->partialBeans === true || (is_array($this->partialBeans) && in_array($table,
                            $this->partialBeans)));
                if ($partial) {
...
namespace RedBeanPHP {
    ...
    class Facade
    {
        ...
        public static function loadJoined($beans, $type, $sqlTemplate = 'SELECT %s.* FROM %s WHERE id IN (%s)')
        {
            ...
            $result = self::findMulti($type, self::genSlots($beans, sprintf($sqlTemplate, $type, $type, '%s')), $ids,
                array(Finder::onmap($type, $beans)));
            $bean = reset($beans);
...
namespace RedBeanPHP\Util {
    ...
    class Diff
    {
        ...
        public function diff(
            $beans,
            $others,
            $filters = array('created', 'modified'),
            $format = '%s.%s.%s',
            $type = null
        ) {
            $diff = array();
